...
public class DeployVDocSDKPomMojo extends AbstractVDocMojo
{
	...
	public void execute() throws MojoExecutionException, MojoFailureException
	{
		...
		{
			deploymentRepository = Repository.createRepository(this.project.getDistributionManagement().getSnapshotRepository());
		}
		else
		{
			deploymentRepository = Repository.createRepository(this.project.getDistributionManagement().getRepository());
		}
		
		maven = new Maven(this.session, this.project, (PluginDescriptor) this.getPluginContext().get("pluginDescriptor"), this.mavenHome);
	...
	protected void buildParentPom(ParentPOM pom) throws MojoExecutionException, MojoFailureException
	{
		...
		try
		{
			ParentPOMGenerator generator = new ParentPOMGeneratorImpl(this.maven, pom,  this.getProject().getVersion());
			File pomFile = generator.generate();
			
			Artifact artifact = new Artifact(pomFile, pom.getArtifactId(), this.getProject().getVersion(), "com.vdoc.engineering");
			artifact.setPackaging("pom");
			DeployFileConfiguration deployFileConfiguration = new DeployFileConfiguration(maven, deploymentRepository, artifact);
