    ...
    try {
      ...
      try (
          InputStream input = this.getClass().getClassLoader()
              .getResourceAsStream("documentation/" + ftlName);
          FileOutputStream outputStream = new FileOutputStream(
              new File(this.tempDirectory, ftlName))
      ) {
      ...
      for (MojoDescriptor mojoDescriptor : this.pluginDescriptor.getMojos()) {
        LOGGER.info("Build documentation into " + this.outputDirectory.getAbsolutePath() + '/'
            + mojoDescriptor.getGoal() + ".html");
